...
impl ProgManager {
    ...
    pub fn _affect_to(&self, addr: usize) -> String {
        ...
        format!(
            "\nmov {}[_stack + {STACK_REG} + {}], {}",
            ASM_SIZES[size], addr, RAX_SIZE[size]
        )
    }
...
add rax, {}
mov {}[_stack + {STACK_REG} + {}], {}",
                val, ASM_SIZES[size], addr, RAX_SIZE[size]
            )
        } else {
            format!(
                "\nmov {}[_stack + {STACK_REG} + {}], {}",
                ASM_SIZES[size], addr, val
            )
        }
    }
    ...
    pub fn deref_var(&self, size: usize, stars: i32, spot: MemZone) -> String {
        if stars > 0 {
            match spot {
                MemZone::Heap => {
                    format!("mov {}, {}[_heap + rax]", RAX_SIZE[size], ASM_SIZES[size])
                }
                MemZone::Stack => {
...
mov {}[_stack + {STACK_REG} + {}], {}",
    ...
    pub fn panic_bad_token(&self, receiver: &str, token: Token) {
        panic!(
            "Line {}: Unknow token type for a {receiver}: {:?}    {}",
            self.line_number(),
            token.token_type,
            token.content
        )
